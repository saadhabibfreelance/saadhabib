    ...
    <Section index={2} id="stats">
      <div className="text-center text-white">
        <h2 data-reveal="mask" className="text-4xl font-black tracking-tight text-white sm:text-6xl">
          Numbers that <span className="text-[#C8A96A]">deliver</span>.
        </h2>
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((s, i) => (
            <div
              ...
              className="rounded-3xl border border-white/30 bg-white/10 p-8 backdrop-blur transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-2 hover:border-[#C8A96A] hover:bg-white/15 hover:shadow-[0_20px_50px_-10px_rgba(255,238,0,0.35)]"
            >
              <div className="text-6xl font-black text-[#C8A96A] drop-shadow">{s.value}</div>
            ...
            <div key={i} className="flex gap-12">
              {marqueeWords.map((w) => (
                <span key={w} className="flex items-center gap-12">
                  {w} <span className="text-[#C8A96A]">✦</span>
                </span>
              ))}
            </div>
          ))}
